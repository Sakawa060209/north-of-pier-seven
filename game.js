...
E075|赵氏地产名单|C|赵氏地产|三名受害者与拆迁均有松散关系，但不足以解释林凯。|2`;
  ...
  const findings = {
    ...
    F06:{name:"第五案现场异常",desc:"日期、电话、相纸、数字与布置五项均偏离前四案稳定模式。"},
  ...
  function freshState(){return {chapter:1,evidence:[],evidenceLog:{},findings:[],deductions:[],visited:[],spots:[],puzzles:[],interviews:{},selected:[],notes:"",pressure:0,pressureEvents:[],wrong:0,sunSafe:false,sunOutcome:"pending",sunDeadline:null,timeline:false,photo:false,ending:null,actions:0,wastedActions:0,gameMinutes:1180,destroyedE059:false,intake:[]};}
  ...
  function renderMap(){root.innerHTML=`<div class="section-intro"><p>地图只显示当前档案已确认的地点。○ 为未调查，✓ 为已调查，红色 ! 表示旧地点出现了新的调查方向。</p><span class="tag">已访问 ${state.visited.length} / ${locations.filter(l=>l.ch<=state.chapter).length}</span></div><div class="map-board">${locations.filter(l=>l.ch<=state.chapter).map(l=>{const i=locations.indexOf(l),fresh=locationHasNew(l);return `<button class="map-node ${state.visited.includes(l.id)?"visited":""} ${fresh?"new":""}" data-loc="${l.id}" style="left:${Math.min(l.x,82)}%;top:${Math.min(l.y,88)}%">${fresh?"!":state.visited.includes(l.id)?"✓":"○"} ${String(i+1).padStart(2,"0")} ${l.name}</button>`}).join("")}</div>`;}
  function renderLocation(id){const l=locations.find(x=>x.id===id);if(!l)return;activeLocation=id;if(!state.visited.includes(id)){state.visited.push(id);action({minutes:18});save();}
  ...
  function inspectSpot(index){const l=locations.find(x=>x.id===activeLocation),s=l.spots[index],key=spotKey(l.id,index);if(!state.spots.includes(key))state.spots.push(key);const eligible=s.ids.filter(id=>evidenceChapter(id)<=state.chapter&&id!=="E057"&&!(id==="E059"&&state.destroyedE059));save();
    ...
    const items=eligible.map(id=>evidence[id]).filter(Boolean);if(items.length>1){openModal("现场勘查 / "+s.n,`<span class="stamp">${l.name}</span><h3>${s.n}</h3><p>该区域包含 ${items.length} 项可独立查看的材料。逐件检查后才能入库。</p><div class="grid grid-2">${items.map(e=>`<div class="card"><span class="file-ref">${e.id}</span><h3>${e.name}</h3><p>${has(e.id)?"已完成取证":"尚未查看正文"}</p><button class="action" data-collect-evidence="${e.id}">${has(e.id)?"重新查看":"检查并入库"}</button></div>`).join("")}</div>`);}else if(items.length===1){addEvidence([items[0].id],false);openModal("现场勘查 / "+s.n,`<span class="stamp">${l.name}</span><h3>${s.n}</h3><div class="document"><span class="file-ref">${items[0].id} / 等级 ${items[0].grade}</span><h3>${items[0].name}</h3><p>${items[0].desc}</p></div>`);}else openModal("现场勘查 / "+s.n,`<span class="stamp">${l.name}</span><h3>${s.n}</h3><p>当前阶段未发现可独立入库的证物。环境观察已经记录。</p>`);renderLocation(l.id);}
  ...
  function renderEvidence(){
    ...
    const findingCards=state.findings.map(id=>findings[id]&&`<article class="card evidence-card link"><span class="file-ref">${id} / ANALYSIS</span><h3>${findings[id].name}</h3><p>${findings[id].desc}</p></article>`).filter(Boolean).join("");
    const intro=`<div class="section-intro"><p>原始证物只记录可观察事实；完成比对后产生鉴定记录，再由多来源材料形成推论。A 为核心、B 为关联、C 为合理误导。</p><span class="tag">${owned.length} / 75 已入库 · ${state.findings.length} 项鉴定</span></div>${findingCards?`<h3 class="section-label">鉴定记录</h3><div class="grid grid-3" style="margin-bottom:20px">${findingCards}</div><h3 class="section-label">原始证物</h3>`:""}`;
  ...
  function solveCaseCompare(){const checked=$$("[data-case-anomaly]:checked").length,nature=$("#case5-nature").value;if(checked===5&&nature==="silence"){addFinding("F06");addDeduction("T05");addPuzzle("10");closeModal();toast("五案比较完成");render();}else{state.wrong++;action({minutes:15,pressure:1,waste:true});toast(checked<5?`仍有 ${5-checked} 项异常未标记`:"异常识别正确，但案件性质仍未解释");}}
