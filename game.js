...
(() => {
  ...
  const chapters = [
    ...
    {n:3,title:"第四个人",date:"2004 / 12 / 10",intro:"摄影师林凯成为数字 4。相同的黑色相纸第一次有了自然来源，而他外套中的空底片盒写着：92 / 12 / 18 / 北七。",dispatch:"先证明相纸从哪里来，再判断林凯为何不是随机的第四人。",objectives:["勘查河滨仓库","搜查林凯摄影室","确认相纸来源","证明林凯是真正目标"]},
    {n:4,title:"十二年前",date:"2004 / 12 / 11",archiveDate:"1992 / 12 / 18",intro:"北七印刷厂火灾，死亡四人、失踪一人。官方说是电路老化；被拆掉的第 47 页、卡死的机械门和一卷底片却讲述着另一个版本。",dispatch:"当前仍是 2004 年。你正在重审 1992 年档案：先找技术矛盾，再证明人为篡改。",objectives:["调阅 1992 年旧档","发现事故报告的技术矛盾","证明旧案报告遭人为篡改","确认系统性掩盖","恢复完整底片序列"]},
    {n:5,title:"多出来的第五人",date:"2004 / 12 / 12",intro:"记者魏安死于车辆撞击。数字 5 写得仓促，相纸被临时撕下，电话亭没有提前拨号。第五案看似延续，实则处处中断。",dispatch:"逐项标记第五案异常。孙倩的保护申请是限时的可选行动，不会阻止你推进案件。",objectives:["调查第五案路口","破解魏安答录机","检查公交总站维修车","完成五案异常对比","⚠ 申请保护孙倩（可选）"]},
...
E075|赵氏地产名单|C|赵氏地产|三名受害者与拆迁均有松散关系，但不足以解释林凯。|2`;
  ...
  const people = [
    ...
    {id:"xu",ch:3,name:"许文",age:31,job:"二手书店老板",initial:"许",suspicion:36,bio:"火灾遇难者家属，多年收集北七旧案资料。",truth:"重要信息源。他对赵启明的执念既是误导，也引出了账本。",rounds:["我父亲不是死于事故。有人让所有人闭嘴。","林凯来过，他把一张裁切的接触样片留给我保管。","完整照片不在我这里。但林凯说过：只看一张，会把救人者当成凶手。"]},
    ...
    {id:"huang",ch:2,name:"黄启",age:38,job:"17 路公交司机",initial:"启",suspicion:18,bio:"末班车司机，当前身份为普通目击证人。",truth:"黄志远之子，也是五案真凶。因误读裁切照片而杀害唯一能证明父亲清白的人。",rounds:["开久了，谁几点上车、几点回家，我大概都知道。但我不认识死者。","排班不是我定的。总站附近那么多人……我不认识林凯。","我只看到他进仓。他们会再说我爸纵火。我不能让那卷照片出去。"]},
  ...
  const deductions = [
    ...
    {id:"T08B",name:"旧案报告遭人为篡改",need:["E050","E059"],needsD:["T08A"],desc:"损坏方向与被删记录证明报告被人为改写。"},
    {id:"T08C",name:"旧案存在系统性掩盖",need:["E052","E053"],needsD:["T08B"],desc:"非法账本、异常付款与报告篡改形成完整掩盖链。"},
    {id:"T09",name:"17 路观察者假说",need:["E005","E021","E061"],desc:"司机可长期观察三名伪装受害者。"},
    {id:"T10",name:"黄志远破门救人",need:["E050","E055"],desc:"破坏方向与幸存者证词相互印证。"}
  ];

  function freshState(){return {chapter:1,evidence:[],evidenceLog:{},deductions:[],visited:[],spots:[],puzzles:[],interviews:{},selected:[],notes:"",pressure:12,pressureEvents:[],wrong:0,sunSafe:false,sunOutcome:"pending",sunDeadline:null,timeline:false,photo:false,ending:null,actions:0,gameMinutes:1180,destroyedE059:false};}
  ...
  function load(){try{const parsed=JSON.parse(localStorage.getItem(SAVE_KEY));if(parsed){state={...freshState(),...parsed};state.evidenceLog={...(parsed.evidenceLog||{})};if(state.deductions.includes("T04")&&!hasD("T04A"))state.deductions.push("T04A");if(state.deductions.includes("T08")&&!hasD("T08B"))state.deductions.push("T08A","T08B");}}catch{} }
  ...
  function openModal(label,html){openModal.returnFocus=document.activeElement;$("#modal-label").textContent=label;modalBody.innerHTML=html;modal.showModal();setTimeout(()=>modalBody.querySelector("button,input,select")?.focus(),0);}
  ...
  function action(cost=1,minutes=18){state.actions+=cost;state.gameMinutes+=minutes;state.pressure=Math.min(40,state.pressure+cost);checkPressure();if(state.chapter===5&&state.sunOutcome==="pending"&&state.sunDeadline!==null&&state.actions>=state.sunDeadline){state.sunOutcome=state.pressure>=30?"dead":"injured";toast(state.sunOutcome==="dead"?"紧急通报：孙倩在总站失联":"紧急通报：孙倩独自调查时受伤");}save();if(!shell.hidden)updateChrome();}
  function checkPressure(){const events=[
    {at:20,id:"media",text:"媒体正式使用“七码头连环杀手”称呼，调查压力上升。"},
    {at:26,id:"superior",text:"上级要求提交初步嫌疑方向，错误指认的代价将提高。"},
    {at:32,id:"sun",text:"孙倩决定独自前往公交总站；第五章保护窗口将十分有限。"},
    {at:36,id:"destroy",text:"周成开始清理旧档。若原始修改报告尚未入库，只能通过审讯恢复。"},
    {at:40,id:"force",text:"专案组达到最高压力：进入第六章后将被要求尽快提交报告。"}
  ...
  function renderDesk(){const ch=chapters[state.chapter-1];const victims=["唐辉 · 11/19 · 重击","吴峰 · 11/26 · 溺水","方蓉 · 12/03 · 窒息","林凯 · 12/10 · 失血","魏安 · 12/12 · 车辆撞击"];
    root.innerHTML=`<div class="chapter-hero"><div><p class="eyebrow">CHAPTER ${String(ch.n).padStart(2,"0")}</p><h3>${ch.title}</h3><p>${ch.intro}</p></div><div class="chapter-number">${ch.n}</div></div>
      ...
      <div class="card" style="margin-top:15px"><span class="file-ref">ACTIVE THEORY</span><h3>${hasD("T04B")?"连环案可能是伪装":hasD("T04A")?"相纸来源已确认":hasD("T03")?"受害者因可预测而被选中":"共同模式尚未解释"}</h3><p>${hasD("T04B")?"真正目标被隐藏在三个随机死者之中。":hasD("T04A")?"来源相同不等于犯罪结构已经解释，还要判断凶手为何寻找底片。":hasD("T03")?"职业、年龄与社会关系不同；固定生活节奏才是共同点。":"不要急着给数字和相纸赋予意义。"}</p></div></section></div>${state.pressureEvents.length?`<div style="margin-top:15px">${state.pressureEvents.map(id=>{const m={media:"媒体已命名本案，证人开始受报道影响。",superior:"上级要求初步嫌疑方向，错误指认将增加更多压力。",sun:"孙倩已经准备独自调查，保护窗口将在第五章开启。",destroy:"周成已经清理部分旧档，缺失报告需从口供恢复。",force:"专案组已达到最高压力，第六章将被要求尽快结案。"};return `<div class="pressure-event">${m[id]}</div>`}).join("")}</div>`:""}`;
  }
  function intake(){const packs={1:["E001"],2:["E009","E017"],3:["E025"],4:["E047"],5:["E037"],6:["E061"]};addEvidence(packs[state.chapter]);action();render();}
  function advance(){if(!canAdvance())return;if(state.chapter===5&&state.sunOutcome==="pending")state.sunOutcome=state.pressure>=30?"dead":"injured";if(state.chapter<6){state.chapter++;const starts={2:1270,3:1230,4:1175,5:1100,6:1160};state.gameMinutes=starts[state.chapter];action(2,0);if(state.chapter===5)state.sunDeadline=state.actions+3;save();toast(`新章节解锁：${chapters[state.chapter-1].title}`);navigate("desk");}}

  function locationHasNew(l){if(!state.visited.includes(l.id))return false;const unlocked=l.spots.some(s=>s.ids.some(id=>evidenceChapter(id)<=state.chapter&&!has(id)&&id!=="E057"&&!(id==="E059"&&state.destroyedE059)));return unlocked||(l.id==="studio"&&state.chapter>=4&&!state.photo)||(l.id==="news"&&state.chapter>=5&&!has("E043"));}
  function spotHasNew(l,s){return s.ids.some(id=>evidenceChapter(id)<=state.chapter&&!has(id)&&id!=="E057"&&!(id==="E059"&&state.destroyedE059))||(l.id==="studio"&&s.n==="墙柜暗格"&&state.chapter>=4&&!state.photo)||(l.id==="news"&&s.n==="答录机"&&!has("E043"));}
  function renderMap(){root.innerHTML=`<div class="section-intro"><p>地点节点随案件推进解锁。○ 为未调查，✓ 为已调查，红色 ! 表示旧地点出现了新的调查方向。</p><span class="tag">已访问 ${state.visited.length} / 17</span></div><div class="map-board">${locations.map((l,i)=>{const fresh=locationHasNew(l);return `<button class="map-node ${l.ch>state.chapter?"locked":""} ${state.visited.includes(l.id)?"visited":""} ${fresh?"new":""}" data-loc="${l.id}" style="left:${Math.min(l.x,82)}%;top:${Math.min(l.y,88)}%" ${l.ch>state.chapter?"disabled":""}>${fresh?"!":state.visited.includes(l.id)?"✓":"○"} ${String(i+1).padStart(2,"0")} ${l.name}</button>`}).join("")}</div>`;}
  ...
  function inspectSpot(index){const l=locations.find(x=>x.id===activeLocation),s=l.spots[index],key=spotKey(l.id,index);if(!state.spots.includes(key))state.spots.push(key);const eligible=s.ids.filter(id=>evidenceChapter(id)<=state.chapter&&id!=="E057"&&!(id==="E059"&&state.destroyedE059));addEvidence(eligible,false);save();
    if(l.id==="news"&&index===1&&!has("E043")){showAnswerMachine();return;}
    if(l.id==="studio"&&index===3&&!state.photo){if(state.chapter<4){openModal("墙柜暗格",`<h3>一卷严重曝光的底片</h3><p>当前设备无法恢复影像。需要先调阅 1992 年档案，建立参照时间。</p>`);renderLocation(l.id);return;}startPuzzle("08");return;}
  ...
  const interviewNeeds={
    ...
    sun:{2:["E042","E043"],3:["E043","E057","E072"]},zhao:{2:["E052"],3:["D:T08B","E052","E053"]},huang:{2:["D:T09","E062","E063"],3:["E032","E034","E057","E063","E073"]},lin:{2:["E056"],3:["E051","E056","E071"]}
  };
  ...
  function renderPeople(){const visible=people.filter(p=>p.ch<=state.chapter);root.innerHTML=`<div class="section-intro"><p>人物档案随调查逐步加入。每轮审讯都需要能击穿当前口供的证据，无法通过连续点击提前获得结论。</p><span class="tag amber">口供可信度 ★★</span></div><div class="grid grid-3">${visible.map((p,i)=>{const r=state.interviews[p.id]||0,g=interviewGate(p.id,Math.min(r+1,3));return `<article class="card person-card"><div class="portrait">${p.initial}</div><span class="person-index">${String(i+1).padStart(2,"0")}</span><h3>${p.name}</h3><p>${p.age} 岁 / ${p.job}</p><p>${p.bio}</p><div>${r?`<span class="tag">已审讯 ${r}/3</span>`:"<span class='tag dim'>尚未审讯</span>"}${p.id==="huang"&&r<2?"<span class='tag'>普通证人</span>":""}</div><div class="suspect-meter"><i style="width:${p.suspicion}%"></i></div><div class="action-row"><button class="action" data-person="${p.id}" ${r<3&&!g.ok?"disabled":""}>${r>=3?"查看结论":`进行第 ${r+1} 轮`}</button></div>${r<3&&!g.ok?`<p class="file-ref">需补充 ${g.missing.length} 项矛盾材料</p>`:""}</article>`}).join("")}</div>`;}
  function interview(id){const p=people.find(x=>x.id===id);let r=state.interviews[id]||0;if(r>=3){openModal("人物结论",`<h3>${p.name}</h3><p>${p.truth}</p>`);return;}const gate=interviewGate(id,r+1);if(!gate.ok){openModal("审讯条件不足",`<div class="gate-box"><h3>当前口供无法被击穿</h3><p>还缺少能直接形成矛盾的材料：${gate.missing.join("、")}。</p></div>`);return;}r++;state.interviews[id]=r;action(1,25);if(id==="zhou"&&r===3&&state.destroyedE059)addEvidence(["E059"],true,true);save();openModal(`审讯记录 / 第 ${r} 轮`,`<span class="stamp">口供可信度 ★★</span><h3>${p.name}</h3><p>“${p.rounds[r-1]}”</p><div class="callout">调查员批注：${r===1?"记录陈述，暂不采信。":r===2?"口供与出示证据形成可核查矛盾。":"证据链完成，最终陈述已归档。"}</div>`);renderPeople();}
  ...
  function viewEvidence(id){const e=evidence[id],log=state.evidenceLog[id]||{};openModal(`证物档案 / ${id}`,`<span class="stamp">等级 ${e.grade}</span><h3>${e.name}</h3><div class="evidence-meta"><div><small>发现地点</small><b>${e.source}</b></div><div><small>取得时间</small><b>${log.chapter?`第 ${log.chapter} 章 / ${log.time}`:"旧存档记录"}</b></div><div><small>可信度</small><b>${"★".repeat(e.stars)}${"☆".repeat(5-e.stars)}</b></div><div><small>证据用途</small><b>${e.grade.startsWith("A")?"核心证据":e.grade==="B"?"关联证据":"误导方向"}</b></div></div><div class="document"><p>${e.desc}</p></div><div class="callout">调查员批注：${e.grade==="C"?"该材料可以引出调查方向，但不能单独支持指认。":"应与时间、来源或人物口供进行交叉验证。"}</div><div class="action-row"><button class="action primary" data-evidence-add-from-modal="${id}">${state.selected.includes(id)?"从推理板移除":"加入推理板"}</button></div>`);}
  ...
  function combine(){const ready=d=>!d.needsD||d.needsD.every(hasD);const match=deductions.find(d=>!hasD(d.id)&&ready(d)&&d.need.length===state.selected.length&&d.need.every(id=>state.selected.includes(id)));if(match){addDeduction(match.id);state.selected=[];save();renderEvidence();return;}const candidates=deductions.filter(d=>!hasD(d.id)&&ready(d)&&d.need.some(id=>state.selected.includes(id))).map(d=>({d,hit:d.need.filter(id=>state.selected.includes(id)),missing:d.need.filter(id=>!state.selected.includes(id)),extra:state.selected.filter(id=>!d.need.includes(id))})).sort((a,b)=>b.hit.length-a.hit.length);const near=candidates[0];state.wrong++;action(1,15);let feedback="当前证物之间缺少可验证的直接联系。";if(near?.hit.length){feedback=`方向接近“${near.d.name}”。你已经选对 ${near.hit.length} 条支持材料，仍缺少 ${near.missing.length} 条${near.missing.length?"不同来源的证据":""}。${near.extra.length?`当前选择中有 ${near.extra.length} 条没有支持该结论，请先剔除。`:""}`;}openModal("组合分析",`<h3>论证尚未闭合</h3><p>${feedback}</p>${near?.extra.some(id=>evidence[id]?.grade==="C")?`<div class="callout">其中包含合理误导材料。它能引出方向，但不能作为这条结论的核心支撑。</div>`:""}`);updateChrome();}
  ...
  const timelineRequired=["E001","E005","E008","E009","E013","E014","E017","E021","E024","E025","E030","E062","E063"];
  function renderTimeline(){if(state.chapter<6){root.innerHTML=`<div class="empty">四案排班重建将在最终章开放。</div>`;return;}const missing=timelineRequired.filter(id=>!has(id));if(missing.length){root.innerHTML=`<div class="gate-box"><h3>时间线材料不足</h3><p>你还缺少 ${missing.length} 项来自尸检、死者日程、电话或公交总站的时间记录。四案必须全部核对，不能以两案代替。</p></div>`;return;}let offset=0;root.innerHTML=`<div class="section-intro"><p>逐案重建“回站 → 电话 → 受害者出现 → 死亡”的完整链条，亲自验证四个机会窗口。</p><span class="tag ${state.timeline?"amber":""}">${state.timeline?"四案已验证":"待重建"}</span></div>${timelineCases.map(c=>{const times=[...new Set(c.events.map(e=>e.a).concat(c.events.map(e=>{const [h,m]=e.a.split(":").map(Number);return `${String(h).padStart(2,"0")}:${String((m+6)%60).padStart(2,"0")}`})))].sort();const html=`<div class="card" style="margin-bottom:16px"><h3>${c.name}</h3><div class="timeline-form">${c.events.map((x,j)=>{const i=offset+j;return `<div class="timeline-event" data-time="${j+1}"><b>${x.e}</b><select data-time-select="${i}"><option value="">选择时间</option>${times.map(t=>`<option ${state.timeline&&t===x.a?"selected":""}>${t}</option>`).join("")}</select></div>`}).join("")}</div></div>`;offset+=c.events.length;return html}).join("")}<div class="action-row"><button class="action primary" data-check-timeline>核对四案时间线</button></div>`;}
  function checkTimeline(){const vals=$$("[data-time-select]").map(s=>s.value);if(vals.every((v,i)=>v===timelineEvents[i].a)){state.timeline=true;addPuzzle("13");addDeduction("T09");save();toast("四案时间线验证完成：黄启每次都具备机会窗口");renderTimeline();}else{state.wrong++;action(1,15);toast("至少一案的时间顺序存在冲突，请分别核对尸检与回站记录");}}

  const puzzles=[
    {id:"01",ch:1,name:"公交票残角",requires:["E002","E003"],suggest:"电影院后巷",q:"残票属于哪条线路？",opts:["12 路","17 路","23 路"],a:1,reward:["E004"]},
    {id:"02",ch:1,name:"唐辉行动路线",requires:["E005"],suggest:"唐辉酒吧",q:"唐辉被选择的关键是什么？",opts:["债务","固定下班路线","酒吧身份"],a:1,d:"T01"},
    {id:"03",ch:2,name:"假维修地址",requires:["E012","E013"],suggest:"地下排水通道",q:"北七巷 17-4 的核查结果？",opts:["已拆迁","从未存在","门牌错误"],a:1,reward:["E013"]},
    {id:"04",ch:2,name:"前三人共同点",requires:["E005","E013","E021"],suggest:"三名死者的日常记录",q:"职业年龄各异，真正共同点是？",opts:["反拆迁","旧案亲属","固定时间、路线且独行"],a:2,d:"T03"},
    {id:"05",ch:3,name:"黑色相纸与真正目标",requires:["E003","E011","E019","E027","E028","E029","E033"],suggest:"林凯摄影工作室与河滨仓库",custom:"paper"},
    {id:"06",ch:4,name:"1992 档案缺页",requires:["E047","E048"],suggest:"警局旧档",q:"目录 46 后直接是 48，装订孔有撕裂。",opts:["自然脱落","编号错误","人为拆除"],a:2,reward:["E048"]},
    ...
    {id:"09",ch:4,name:"财务账本",requires:["E052"],suggest:"印刷厂旧址",q:"ZC-92-12 最终流向？",opts:["陈岚诊所","周成关联账户","林正国报社"],a:1,reward:["E053"]},
    ...
    {id:"12",ch:5,name:"公交线路覆盖",requires:["E004","E005","E021","E061"],suggest:"公交总站与前三案日程",q:"哪条路线覆盖全部案发地点？",opts:["12 路","17 路","31 路"],a:1,d:"T09"},
    {id:"13",ch:6,name:"四案排班时间窗口",requires:["E001","E005","E008","E009","E013","E014","E017","E021","E024","E025","E030","E062","E063"],suggest:"尸检、电话记录与总站调度室",custom:"timeline"},
    {id:"14",ch:6,name:"第五案车辆匹配",requires:["E040","E044"],suggest:"第五案路口与总站车库",q:"轮胎、油漆与缺失记录对应哪辆车？",opts:["维修 A 车","维修 B 车","维修 C 车"],a:1,reward:["E045","E067"]},
    {id:"15",ch:6,name:"最终案件重建",requires:["E057","E073"],requiresD:["T04B","T05","T09"],suggest:"完整底片、黄启关系与核心推论",custom:"report"}
  ];
  function puzzleGate(p){const missing=(p.requires||[]).filter(id=>!has(id)).map(id=>evidence[id]?.name);const missingD=(p.requiresD||[]).filter(id=>!hasD(id)).map(id=>deductions.find(d=>d.id===id)?.name);return {ok:!missing.length&&!missingD.length,missing:[...missing,...missingD].filter(Boolean)};}
  ...
  function answerPuzzle(pid,choice){if(pid==="05a"){if(+choice===1){addDeduction("T04A");openModal("谜题 05 / 第二步",`<h3>凶手为何只取走林凯相机中的胶卷？</h3><p>相纸来源已经确认，但这还不能证明犯罪结构。</p><div class="action-row"><button class="action" data-answer="05b:0">林凯只是随机的第四名死者</button><button class="action" data-answer="05b:1">林凯是目标，前三案复制了他的特征</button></div>`);}else wrongPuzzle();return;}if(pid==="05b"){if(+choice===1){addDeduction("T04B");addPuzzle("05");closeModal();toast("两步推理完成：真正目标被隐藏");render();}else wrongPuzzle();return;}const p=puzzles.find(x=>x.id===pid);if(+choice===p.a){addPuzzle(pid);if(p.reward)addEvidence(p.reward);if(p.d)addDeduction(p.d);closeModal();toast(`谜题完成：${p.name}`);render();}else wrongPuzzle();}
  function wrongPuzzle(){state.wrong++;action(1,15);toast("这个结论无法解释全部证据");updateChrome();}
  ...
  function showCaseCompare(){const rows=["案发日期不是星期五","没有事前电话","相纸为临时撕裂","数字字迹明显不同","现场没有预先布置"];openModal("谜题 10 / 五案异常对比",`<h3>标记 CASE 05 与前四案不同的项目</h3><table class="case-compare"><thead><tr><th>特征</th><th>CASE 01—04</th><th>CASE 05 异常</th></tr></thead><tbody>${rows.map((r,i)=>`<tr><td>${r}</td><td class="normal">规律一致</td><td><label><input type="checkbox" data-case-anomaly="${i}"> 标记</label></td></tr>`).join("")}</tbody></table><label>完成比较后判断性质</label><select id="case5-nature"><option value="">选择结论</option><option value="copy">模仿犯罪</option><option value="silence">临时灭口</option><option value="accident">交通意外</option></select><div class="action-row"><button class="action primary" data-case-compare-submit>提交异常分析</button></div>`);}
  function solveCaseCompare(){const checked=$$("[data-case-anomaly]:checked").length,nature=$("#case5-nature").value;if(checked===5&&nature==="silence"){addDeduction("T05");addPuzzle("10");closeModal();toast("五项异常共同证明：第五案是仓促灭口");render();}else{state.wrong++;action(1,15);toast(checked<5?`仍有 ${5-checked} 项异常未标记`:"异常识别正确，但案件性质仍未解释");}}
  ...
  function solvePhone(){if($("#phone-code").value==="1210"){addPuzzle("11");addEvidence(["E043"]);closeModal();toast("留言已提取为 E043");render();}else{state.wrong++;action(1,15);toast("密码错误");}}
  function showPhotoPuzzle(){openModal("谜题 08 / 底片恢复",`<h3>恢复 1992 年底片序列</h3><p>调整曝光与对比度，使时间戳、人物轮廓和消防门细节同时可辨。过曝或过强对比都会吞掉关键区域。</p><div class="photo-puzzle"><div id="photo-frame" class="photo-frame"></div><div class="puzzle-controls"><label>曝光亮度</label><input id="bright" aria-label="曝光亮度" type="range" min="20" max="100" value="55"><label>对比度</label><input id="contrast" aria-label="对比度" type="range" min="70" max="180" value="100"><div class="photo-readout"><div id="read-time">□ 时间戳不可辨</div><div id="read-figure">□ 两个人物轮廓混在暗部</div><div id="read-door">□ 消防门区域细节丢失</div></div><div class="action-row"><button class="action primary" data-photo-submit>固定显影参数</button></div></div></div>`);bindPhoto();}
  function bindPhoto(){const b=$("#bright"),c=$("#contrast"),f=$("#photo-frame");const mark=(id,ok,yes,no)=>{const el=$(id);el.classList.toggle("ok",ok);el.textContent=(ok?"■ ":"□ ")+(ok?yes:no)};const up=()=>{const bv=+b.value,cv=+c.value;f.style.setProperty("--bright",bv/100);f.style.setProperty("--contrast",cv/100);const reveal=Math.max(.08,1-(Math.abs(bv-80)+Math.abs(cv-140)/2)/70);f.style.setProperty("--reveal",reveal);mark("#read-time",bv>=70&&bv<=90,"时间戳可以辨认","时间戳不可辨");mark("#read-figure",bv>=74&&bv<=86&&cv>=125&&cv<=155,"黄志远与罗晋川轮廓分离","两个人物轮廓混在暗部");mark("#read-door",bv>=76&&bv<=88&&cv>=130&&cv<=150,"消防门与工具痕迹可见","消防门区域细节丢失")};b.oninput=up;c.oninput=up;up();}
  ...
  function submitReport(form){const data=new FormData(form);const raw=reportQs.map((_,i)=>data.get(`q${i}`));if(raw.some(v=>v===""||v===null)){toast("报告仍有未完成项目");return;}const vals=raw.map(Number);const score=vals.filter((v,i)=>v===reportQs[i].a).length;const suspect=vals[9];const hidden=$("#classification").value==="constructed"||$("#report-title").selectedIndex===1;
    const fullOld=["E047","E048","E049","E050","E051","E052","E053","E054","E055","E056","E057","E059","E060"].every(has);const fullLogic=["T04B","T05","T08C","T09","T10"].every(hasD)&&state.timeline;
    let type;if(suspect!==2||score<7)type="C";else if(state.sunOutcome==="dead")type="D";else if(score===10&&state.photo&&hidden&&fullOld&&fullLogic)type="E";else if(score===10&&state.photo)type="A";else type="B";state.ending={type,score,suspect};addPuzzle("15");save();renderEnding();}
  ...
  function protectSun(){if(state.sunSafe){toast("孙倩已进入保护程序");return;}if(state.sunOutcome!=="pending"){toast(state.sunOutcome==="dead"?"保护申请已错过：孙倩失联":"保护申请已错过：孙倩已经受伤");return;}state.sunSafe=true;state.sunOutcome="safe";action(1,12);save();toast("已安排警员保护孙倩");render();}
  ...
  document.addEventListener("click",e=>{
    ...
    if(e.target.closest("[data-phone-submit]")){solvePhone();return;}if(e.target.closest("[data-photo-submit]")){solvePhoto();return;}if(e.target.closest("[data-case-compare-submit]")){solveCaseCompare();return;}if(e.target.closest("[data-check-timeline]")){checkTimeline();return;}
